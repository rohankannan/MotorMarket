...
public class DatabaseTestCase {
    ...
    public void testWriteUserData() {
        ...
        try (Scanner scanner = new Scanner(dbFile)) {
            while (scanner.hasNextLine()) {
                ...
                if (line.contains("user6560") && line.contains("mypassword1") && line.contains("100.0")
                        && line.contains("1 Purdue Drive")) {
                    isWritten = true;
                    break;
                }
            }
    ...
    public void testWriteListingData() {
        ...
        Database db = new Database();
        Listing l = new Listing("seller123", "http://example.com/photo.jpg",
                "Toyota Camry", "Blue", 50000,
        ...
        try (Scanner scanner = new Scanner(dbFile)) {
            while (scanner.hasNextLine()) {
                String line = scanner.nextLine();
                if (line.contains("seller123") && line.contains("http://example.com/photo.jpg")
                        && line.contains("Toyota Camry")
    ...
    public void testReadListingData() {
        ...
        assertEquals("http://example.com/photo.jpg", listings.get(0).getPhotoURL(),
                "The photo URL should match");
        ...
        assertFalse(listings.get(0).isManual(), "The manual status should match");
